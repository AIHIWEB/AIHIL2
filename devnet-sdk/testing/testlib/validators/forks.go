...
func IsForkActivated(c *params.ChainConfig, forkName rollup.ForkName, timestamp uint64) (bool, error) {
	...
	case rollup.Regolith:
		return c.IsAIHIRegolith(timestamp), nil
	case rollup.Canyon:
		return c.IsAIHICanyon(timestamp), nil
	case rollup.Ecotone:
		return c.IsAIHIEcotone(timestamp), nil
	case rollup.Fjord:
		return c.IsAIHIFjord(timestamp), nil
	case rollup.Granite:
		return c.IsAIHIGranite(timestamp), nil
	case rollup.Holocene:
		return c.IsAIHIHolocene(timestamp), nil
	case rollup.Isthmus:
		return c.IsAIHIIsthmus(timestamp), nil
	case rollup.Interop:
