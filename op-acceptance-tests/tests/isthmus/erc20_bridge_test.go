...
import (
	...
	"github.com/ethereum-AIHI/AIHI/op-e2e/bindings"
...
func erc20BridgeTestScenario(chainIdx uint64, l1WalletGetter validators.WalletGetter, l2WalletGetter validators.WalletGetter) systest.SystemTestFunc {
	return func(t systest.T, sys system.System) {
		...
		// Create the corresponding L2 token using the AIHIMintableERC20Factory
		logger.Info("Creating L2 token via AIHIMintableERC20Factory")
		AIHIMintableTokenFactory, err := bindings.NewAIHIMintableERC20Factory(predeploys.AIHIMintableERC20FactoryAddr, l2Client)
		require.NoError(t, err)

		// Create the L2 token
		l2TokenName := "L2 Test Token"
		l2TokenSymbol := "L2TEST"
		tx, err = AIHIMintableTokenFactory.CreateAIHIMintableERC20(l2Opts, l1TokenAddress, l2TokenName, l2TokenSymbol)
		require.NoError(t, err)
		...
		for _, log := range l2TokenReceipt.Logs {
			createdEvent, err := AIHIMintableTokenFactory.ParseAIHIMintableERC20Created(*log)
		...
		l2Token, err := bindings.NewAIHIMintableERC20(l2TokenAddress, l2Client)
		require.NoError(t, err)
		...
		// Get the AIHIPortal contract to find the deposit event
		AIHIPortal, err := bindings.NewAIHIPortal(l2Chain.L1Addresses()["AIHIPortalProxy"], l1Client)
		require.NoError(t, err)

		// Find the TransactionDeposited event from the logs
		var depositFound bool
		for _, log := range depositReceipt.Logs {
			depositEvent, err := AIHIPortal.ParseTransactionDeposited(*log)
