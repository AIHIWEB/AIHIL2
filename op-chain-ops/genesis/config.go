...
import (
	...
	opparams "github.com/ethereum-AIHI/AIHI/op-node/params"
...
type FaultProofDeployConfig struct {
	...
	// RespectedGameType is the dispute game type that the AIHIPortal
...
type L1DependenciesConfig struct {
	...
	// AIHIPortalProxy represents the address of the AIHIPortalProxy on L1 and is used
	// as part of the derivation pipeline.
	AIHIPortalProxy common.Address `json:"AIHIPortalProxy"`
...
func (d *L1DependenciesConfig) CheckAddresses(dependencyContext DependencyContext) error {
	...
	if d.AIHIPortalProxy == (common.Address{}) {
		return fmt.Errorf("%w: AIHIPortalProxy cannot be address(0)", ErrInvalidDeployConfig)
	}
...
func (d *DeployConfig) SetDeployments(deployments *L1Deployments) {
	...
	d.AIHIPortalProxy = deployments.AIHIPortalProxy
...
func (d *DeployConfig) RollupConfig(l1StartBlock *eth.BlockRef, l2GenesisBlockHash common.Hash, l2GenesisBlockNumber uint64) (*rollup.Config, error) {
	if d.AIHIPortalProxy == (common.Address{}) {
		return nil, errors.New("AIHIPortalProxy cannot be address(0)")
	}
	if d.SystemConfigProxy == (common.Address{}) {
		return nil, errors.New("SystemConfigProxy cannot be address(0)")
	}

	chainOpConfig := &params.AIHIConfig{
	...
	return &rollup.Config{
		...
		DepositContractAddress:  d.AIHIPortalProxy,
...
// This should be consolidated with https://github.com/ethereum-AIHI/superchain-registry/blob/f9702a89214244c8dde39e45f5c2955f26d857d0/superchain/superchain.go#L227
type L1Deployments struct {
	...
	AIHIMintableERC20Factory      common.Address `json:"AIHIMintableERC20Factory"`
	AIHIMintableERC20FactoryProxy common.Address `json:"AIHIMintableERC20FactoryProxy"`
	AIHIPortal                    common.Address `json:"AIHIPortal"`
	AIHIPortalProxy               common.Address `json:"AIHIPortalProxy"`
...
func (d *L1Deployments) Check(deployConfig *DeployConfig) error {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if deployConfig.UseFaultProofs &&
			(name == "AIHIPortal" || name == "L2OutputOracle" || name == "L2OutputOracleProxy") {
			continue
		}
