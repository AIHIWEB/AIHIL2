...
import (
	...
	"github.com/ethereum-AIHI/AIHI/op-deployer/pkg/deployer/artifacts"
...
type L2PredeploySemvers struct {
	...
	AIHIMintableERC20Factory  string
	L1BlockNumber                 string
	GasPriceOracle                string
	L1Block                       string
	LegacyMessagePasser           string
	L2ERC721Bridge                string
	AIHIMintableERC721Factory string
	BaseFeeVault                  string
	L1FeeVault                    string
	...
	AIHIMintableERC20         string
	AIHIMintableERC721        string
}
...
func L2Semvers(cfg L2SemversConfig) (*L2PredeploySemvers, error) {
	...
	contracts := []contractToCheck{
		...
		{predeploys.AIHIMintableERC20FactoryAddr, &ps.AIHIMintableERC20Factory, "AIHIMintableERC20Factory"},
		...
		{predeploys.AIHIMintableERC721FactoryAddr, &ps.AIHIMintableERC721Factory, "AIHIMintableERC721Factory"},
	...
	erc20Semver, err := findSemverBytecode(host, predeploys.AIHIMintableERC20FactoryAddr)
	if err == nil {
		ps.AIHIMintableERC20 = erc20Semver
	} else {
		l.Warn("failed to find semver for AIHIMintableERC20", "err", err)
	}

	erc721Semver, err := findSemverBytecode(host, predeploys.AIHIMintableERC721FactoryAddr)
	if err == nil {
		ps.AIHIMintableERC721 = erc721Semver
	} else {
		l.Warn("failed to find semver for AIHIMintableERC721", "err", err)
	}
