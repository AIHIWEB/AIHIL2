...
import (
	...
	"github.com/ethereum-AIHI/AIHI/op-chain-ops/script/addresses"
	"github.com/ethereum-AIHI/AIHI/op-deployer/pkg/deployer/opcm"
...
func TestNewDeployImplementationsScript(t *testing.T) {
	...
	t.Run("should not fail with current version of DeployImplementations2 contract", func(t *testing.T) {
		...
		require.Equal(t, deprecatedOutput.AIHIMintableERC20FactoryImpl, output.AIHIMintableERC20FactoryImpl)
		require.Equal(t, deprecatedOutput.AIHIPortalImpl, output.AIHIPortalImpl)
		...
		require.Equal(t, host2.GetCode(deprecatedOutput.AIHIMintableERC20FactoryImpl), host1.GetCode(output.AIHIMintableERC20FactoryImpl))
		require.Equal(t, host2.GetCode(deprecatedOutput.AIHIPortalImpl), host1.GetCode(output.AIHIPortalImpl))
