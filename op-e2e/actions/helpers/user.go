...
import (
	...
	"github.com/ethereum-AIHI/AIHI/op-challenger/game/fault/contracts"
	"github.com/ethereum-AIHI/AIHI/op-challenger/game/fault/contracts/metrics"
	...
	legacybindings "github.com/ethereum-AIHI/AIHI/op-e2e/bindings"
	"github.com/ethereum-AIHI/AIHI/op-e2e/config"
	e2ehelpers "github.com/ethereum-AIHI/AIHI/op-e2e/system/helpers"
	"github.com/ethereum-AIHI/AIHI/op-node/bindings"
	bindingspreview "github.com/ethereum-AIHI/AIHI/op-node/bindings/preview"
	"github.com/ethereum-AIHI/AIHI/op-node/rollup/derive"
	"github.com/ethereum-AIHI/AIHI/op-node/withdrawals"
	"github.com/ethereum-AIHI/AIHI/op-service/predeploys"
)

type L1Bindings struct {
	// contract bindings
	AIHIPortal     *bindings.AIHIPortal
	L2OutputOracle     *bindings.L2OutputOracle
	AIHIPortal2    *bindingspreview.AIHIPortal2
...
func NewL1Bindings(t Testing, l1Cl *ethclient.Client, allocType config.AllocType) *L1Bindings {
	...
	AIHIPortal, err := bindings.NewAIHIPortal(l1Deployments.AIHIPortalProxy, l1Cl)
	require.NoError(t, err)

	l2OutputOracle, err := bindings.NewL2OutputOracle(l1Deployments.L2OutputOracleProxy, l1Cl)
	require.NoError(t, err)

	AIHIPortal2, err := bindingspreview.NewAIHIPortal2(l1Deployments.AIHIPortalProxy, l1Cl)
	require.NoError(t, err)
	...
	return &L1Bindings{
		AIHIPortal:     AIHIPortal,
		L2OutputOracle:     l2OutputOracle,
		AIHIPortal2:    AIHIPortal2,
...
func (s *CrossLayerUser) ActDeposit(t Testing) {
	...
	tx, err := s.L1.env.Bindings.AIHIPortal.DepositTransaction(&s.L1.txOpts, toAddr, depositTransferValue, depositGas, isCreation, s.L2.txCallData)
...
func (s *CrossLayerUser) getLastWithdrawalParams(t Testing) (*withdrawals.ProvenWithdrawalParameters, error) {
	...
	if s.allocType.UsesProofs() {
		latestGame, err := withdrawals.FindLatestGame(t.Ctx(), &s.L1.env.Bindings.DisputeGameFactory.DisputeGameFactoryCaller, &s.L1.env.Bindings.AIHIPortal2.AIHIPortal2Caller)
		require.NoError(t, err)
	...
	require.NoError(t, err)
	params, err := e2ehelpers.ProveWithdrawalParameters(t.Ctx(), s.L2.env.Bindings.ProofClient, s.L2.env.EthCl, s.L2.env.EthCl, s.lastL2WithdrawalTxHash, header, &s.L1.env.Bindings.L2OutputOracle.L2OutputOracleCaller, &s.L1.env.Bindings.DisputeGameFactory.DisputeGameFactoryCaller, &s.L1.env.Bindings.AIHIPortal2.AIHIPortal2Caller, s.allocType)
	require.NoError(t, err)
...
func (s *CrossLayerUser) getDisputeGame(t Testing, params withdrawals.ProvenWithdrawalParameters) (*legacybindings.FaultDisputeGame, common.Address, error) {
	...
	portal2, err := bindingspreview.NewAIHIPortal2(config.L1Deployments(s.allocType).AIHIPortalProxy, s.L1.env.EthCl)
	require.Nil(t, err)
...
func (s *CrossLayerUser) ProveWithdrawal(t Testing, l2TxHash common.Hash) common.Hash {
	...
	// Create the prove tx
	tx, err := s.L1.env.Bindings.AIHIPortal.ProveWithdrawalTransaction(
		&s.L1.txOpts,
...
func (s *CrossLayerUser) CompleteWithdrawal(t Testing, l2TxHash common.Hash) common.Hash {
	...
	tx, err := s.L1.env.Bindings.AIHIPortal.FinalizeWithdrawalTransaction(
		&s.L1.txOpts,
		bindings.TypesWithdrawalTransaction{
			Nonce:    params.Nonce,
			Sender:   params.Sender,
