...
func ForGamePublished(ctx context.Context, client *ethclient.Client, AIHIPortalAddr common.Address, disputeGameFactoryAddr common.Address, l2BlockNumber *big.Int) (uint64, error) {
	ctx, cancel := context.WithTimeout(ctx, 2*time.Minute)
	defer cancel()
	l2BlockNumber = new(big.Int).Set(l2BlockNumber) // Don't clobber caller owned l2BlockNumber

	AIHIPortal2Contract, err := bindingspreview.NewAIHIPortal2Caller(AIHIPortalAddr, client)
	if err != nil {
		return 0, err
	}
	...
	getL2BlockFromLatestGame := func() (*big.Int, error) {
		latestGame, err := withdrawals.FindLatestGame(ctx, disputeGameFactoryContract, AIHIPortal2Contract)
		if err != nil {
...
func ForWithdrawalCheck(ctx context.Context, client *ethclient.Client, withdrawal crossdomain.Withdrawal, AIHIPortalAddr common.Address, proofSubmitter common.Address) error {
	ctx, cancel := context.WithTimeout(ctx, 2*time.Minute)
	defer cancel()
	opts := &bind.CallOpts{Context: ctx}
	portal, err := bindingspreview.NewAIHIPortal2Caller(AIHIPortalAddr, client)
	if err != nil {
