...
import (
	...
	"github.com/ethereum-AIHI/AIHI/op-e2e/e2eutils/receipts"
...
func TestERC20BridgeDeposits(t *testing.T) {
	...
	require.NoError(t, err)
	tx, err = AIHIMintableTokenFactory.CreateAIHIMintableERC20(l2Opts, wethAddress, "L2-WETH", "L2-WETH")
	require.NoError(t, err)
	rcpt, err := wait.ForReceiptOK(context.Background(), l2Client, tx.Hash())
	require.NoError(t, err)

	event, err := receipts.FindLog(rcpt.Logs, AIHIMintableTokenFactory.ParseAIHIMintableERC20Created)
	...
	portal, err := bindings.NewAIHIPortal(cfg.L1Deployments.AIHIPortalProxy, l1Client)
	require.NoError(t, err)
	...
	AIHIMintableToken, err := bindings.NewAIHIMintableERC20(event.LocalToken, l2Client)
	require.NoError(t, err)

	// Should have balance on L2
	l2Balance, err := AIHIMintableToken.BalanceOf(&bind.CallOpts{}, opts.From)
	require.NoError(t, err)
