...
func testMixedWithdrawalValidity(t *testing.T, allocType config.AllocType) {
	...
	for i := 0; i <= 8; i++ {
		...
		t.Run(fmt.Sprintf("withdrawal test#%d", i+1), func(t *testing.T) {
			...
			var AIHIPortal2 *bindingspreview.AIHIPortal2Caller
			if allocType.UsesProofs() {
				...
				require.NoError(t, err)
				AIHIPortal2, err = bindingspreview.NewAIHIPortal2Caller(cfg.L1Deployments.AIHIPortalProxy, l1Client)
				require.NoError(t, err)
			} else {
			...
			if allocType.UsesProofs() {
				blockNumber, err = wait.ForGamePublished(ctx, l1Client, cfg.L1Deployments.AIHIPortalProxy, cfg.L1Deployments.DisputeGameFactoryProxy, receipt.BlockNumber)
			} else {
			...
			params, err := helpers.ProveWithdrawalParameters(context.Background(), proofCl, receiptCl, blockCl, tx.Hash(), header, l2OutputOracle, disputeGameFactory, AIHIPortal2, cfg.AllocType)
			require.Nil(t, err)
			...
			if badWithdrawal {
				...
				if allocType.UsesProofs() {
					err = wait.ForWithdrawalCheck(ctx, l1Client, withdrawal, cfg.L1Deployments.AIHIPortalProxy, transactor.Account.L1Opts.From)
					require.NoError(t, err)
				} else {
				...
				// Do a large deposit into the AIHIPortal so there's a balance to withdraw
