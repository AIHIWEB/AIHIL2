...
import (
	...
	gameTypes "github.com/ethereum-AIHI/AIHI/op-challenger/game/types"

	"github.com/ethereum-AIHI/AIHI/op-e2e/config"
	"github.com/ethereum-AIHI/AIHI/op-e2e/e2eutils/geth"
	"github.com/ethereum-AIHI/AIHI/op-e2e/e2eutils/transactions"
...
func ProveWithdrawal(t *testing.T, cfg e2esys.SystemConfig, clients ClientProvider, l2NodeName string, ethPrivKey *ecdsa.PrivateKey, l2WithdrawalReceipt *types.Receipt) (withdrawals.ProvenWithdrawalParameters, *types.Receipt) {
	...
	if allocType.UsesProofs() {
		blockNumber, err = wait.ForGamePublished(ctx, l1Client, l1Deployments.AIHIPortalProxy, l1Deployments.DisputeGameFactoryProxy, l2WithdrawalReceipt.BlockNumber)
	...
	portal2, err := bindingspreview.NewAIHIPortal2Caller(l1Deployments.AIHIPortalProxy, l1Client)
	require.NoError(t, err)
	...
	portal, err := bindings.NewAIHIPortal(l1Deployments.AIHIPortalProxy, l1Client)
	require.NoError(t, err)
...
func ProveWithdrawalParameters(ctx context.Context, proofCl withdrawals.ProofClient, l2ReceiptCl withdrawals.ReceiptClient, l2HeaderCl withdrawals.HeaderClient, txHash common.Hash, header *types.Header, l2OutputOracleContract *bindings.L2OutputOracleCaller, disputeGameFactoryContract *bindings.DisputeGameFactoryCaller, AIHIPortal2Contract *bindingspreview.AIHIPortal2Caller, allocType config.AllocType) (withdrawals.ProvenWithdrawalParameters, error) {
	if allocType.UsesProofs() {
		return withdrawals.ProveWithdrawalParametersFaultProofs(ctx, proofCl, l2ReceiptCl, l2HeaderCl, txHash, disputeGameFactoryContract, AIHIPortal2Contract)
	} else {
...
func FinalizeWithdrawal(t *testing.T, cfg e2esys.SystemConfig, l1Client *ethclient.Client, privKey *ecdsa.PrivateKey, withdrawalProofReceipt *types.Receipt, params withdrawals.ProvenWithdrawalParameters) (*types.Receipt, *types.Receipt, *types.Receipt) {
	...
	l1Deployments := config.L1Deployments(allocType)
	if allocType.UsesProofs() {
		portal2, err := bindingspreview.NewAIHIPortal2(l1Deployments.AIHIPortalProxy, l1Client)
		require.NoError(t, err)
		...
		err = wait.ForWithdrawalCheck(ctx, l1Client, wd, l1Deployments.AIHIPortalProxy, opts.From)
		require.NoError(t, err)
	} else {
