...
func FuzzUnmarshallLogEvent(f *testing.F) {
	...
	_, addr, _, err := runtime.Create(common.FromHex(bindings.AIHIPortalMetaData.Bin), &runtime.Config{
		Origin:   from,
		State:    state,
		GasLimit: 20_000_000,
	})
	...
	portalContract, err := bindings.NewAIHIPortal(addr, nil)
	require.NoError(f, err)
	...
	f.Fuzz(func(t *testing.T, _to, _mint, _value, data []byte, l2GasLimit uint64, isCreation bool) {
		...
		reconstructed := &bindings.AIHIPortalTransactionDeposited{
			From:       dep.From,
			Version:    common.Big0,
		...
		inputArgs := &bindings.AIHIPortalTransactionDeposited{
			From:       from,
			To:         to,
			Version:    common.Big0,
			OpaqueData: opaqueData,
