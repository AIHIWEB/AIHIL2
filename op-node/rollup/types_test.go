...
func TestRandomConfigDescription(t *testing.T) {
	...
	t.Run("AIHI forks check,  date", func(t *testing.T) {
		config := randConfig()
		r := uint64(1677119335)
...
func TestConfigImplementsBlockType(t *testing.T) {
	...
	tests := []struct {
		name                       string
		blockTime                  uint64
		hasAIHIWithdrawalsRoot bool
	}{
		{
			name:                       "BeforeIsthmus",
			blockTime:                  uint64(99),
			hasAIHIWithdrawalsRoot: false,
		},
		{
			name:                       "AtIsthmus",
			blockTime:                  uint64(100),
			hasAIHIWithdrawalsRoot: true,
		},
		{
			name:                       "AfterIsthmus",
	...
	for _, test := range tests {
		test := test
		t.Run(fmt.Sprintf("TestHasAIHIWithdrawalsRoot_%s", test.name), func(t *testing.T) {
			assert.Equal(t, config.HasAIHIWithdrawalsRoot(test.blockTime), test.hasAIHIWithdrawalsRoot)
		})
	}
}
