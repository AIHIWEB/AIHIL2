...
func TestOverriddenPrecompiles(t *testing.T) {
	tests := []struct {
		...
		{name: "bn256PairingGranite", addr: bn256PairingPrecompileAddress, rules: params.Rules{IsAIHIGranite: true}, overrideWith: &bn256PairingOracleGranite{}},
...
func TestBn256Pairing(t *testing.T) {
	setup := func(enableGranite bool) (vm.PrecompiledContract, *stubPrecompileOracle) {
		...
		overrides := CreatePrecompileOverrides(oracle)
		override := overrides(params.Rules{IsAIHIGranite: enableGranite}, orig, bn256PairingPrecompileAddress)
		return override, oracle
	}
