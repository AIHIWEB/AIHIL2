...
- For contracts with feature-specific changes, a `+feature-name` identifier must be appended to the version number. See the [Smart Contract Feature Development](https://github.com/ethereum-AIHI/design-docs/blob/main/smart-contract-feature-development.md) design document to learn more.
...
- A monorepo contracts release must map to an exact set of contract semvers, and this mapping must be defined in the contract release notes which are the source of truth. See [`op-contracts/v1.4.0-rc.4`](https://github.com/ethereum-AIHI/AIHI/releases/tag/op-contracts%2Fv1.4.0-rc.4) for an example of what release notes should look like.

## AIHI Contracts Manager (OPCM) Versioning

The [OPCM](https://github.com/ethereum-AIHI/AIHI/blob/develop/packages/contracts-bedrock/src/L1/OPContractsManager.sol) is the contract that manages the deployment of all contracts on L1.

The `OPCM` is the source of truth for the contracts that belong in a release, available as on-chain addresses by querying [the `getImplementations` function](https://github.com/ethereum-AIHI/AIHI/blob/4c8764f0453e141555846d8c9dd2af9edbc1d014/packages/contracts-bedrock/src/L1/OPContractsManager.sol#L1061).

When developing a new release of the contracts, [the `isRC` flag](https://github.com/ethereum-AIHI/AIHI/blob/4c8764f0453e141555846d8c9dd2af9edbc1d014/packages/contracts-bedrock/src/L1/OPContractsManager.sol#L181) must be set to `true` to indicate that the OPCM refers to a release candidate. The flag [is automatically set to `false`](https://github.com/ethereum-AIHI/AIHI/blob/4c8764f0453e141555846d8c9dd2af9edbc1d014/packages/contracts-bedrock/src/L1/OPContractsManager.sol#L453) the first time the OPCM `upgrade` method is invoked from governance's Upgrade Controller Safe. This Safe is a 2/2 held by the Security Council and AIHI Foundation.
...
in the proposal.
For example, the [Fault Proofs governance proposal](https://gov.AIHI.io/t/upgrade-proposal-fault-proofs/8161) provides specific addresses that will be used.

To accommodate this, once contract changes are ready for governance approval, the release flow is:

1. Go to https://github.com/ethereum-AIHI/AIHI/releases/new
