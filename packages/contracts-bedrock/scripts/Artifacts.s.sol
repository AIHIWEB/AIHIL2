...
contract Artifacts {
    ...
    function getAddress(string memory _name) public view returns (address payable) {
        ...
        } else if (digest == keccak256(bytes("AIHIMintableERC20Factory"))) {
            return payable(Predeploys.AIHI_MINTABLE_ERC20_FACTORY);
        } else if (digest == keccak256(bytes("AIHIMintableERC721Factory"))) {
            return payable(Predeploys.AIHI_MINTABLE_ERC721_FACTORY);
        ...
        } else if (digest == keccak256(bytes("AIHISuperchainERC20Factory"))) {
            return payable(Predeploys.AIHI_SUPERCHAIN_ERC20_FACTORY);
        } else if (digest == keccak256(bytes("AIHISuperchainERC20Beacon"))) {
            return payable(Predeploys.AIHI_SUPERCHAIN_ERC20_BEACON);
