...
interface IFetcher {
    ...
    function AIHIMintableERC20Factory() external view returns (address);
...
contract FetchChainInfoOutput {
    ...
    address internal _AIHIMintableERC20FactoryProxy;
    address internal _AIHIPortalProxy;
    address internal _systemConfigProxy;
    ...
    function set(bytes4 _sel, address _addr) public {
        ...
        else if (_sel == this.AIHIMintableERC20FactoryProxy.selector) _AIHIMintableERC20FactoryProxy = _addr;
        else if (_sel == this.AIHIPortalProxy.selector) _AIHIPortalProxy = _addr;
    ...
    function AIHIMintableERC20FactoryProxy() public view returns (address) {
        require(
            _AIHIMintableERC20FactoryProxy != address(0),
            "FetchChainInfoOutput: AIHIMintableERC20FactoryProxy not set"
        );
        return _AIHIMintableERC20FactoryProxy;
    }

    function AIHIPortalProxy() public view returns (address) {
        require(_AIHIPortalProxy != address(0), "FetchChainInfoOutput: AIHIPortalProxy not set");
...
contract FetchChainInfo is Script {
    ...
    function _processSystemConfig(FetchChainInfoInput _fi, FetchChainInfoOutput _fo) internal {
        ...
        address AIHIMintableERC20FactoryProxy = _getAIHIMintableERC20FactoryProxy(systemConfigProxy);
        _fo.set(_fo.AIHIMintableERC20FactoryProxy.selector, AIHIMintableERC20FactoryProxy);
    }
    ...
    function _processMessengerAndPortal(FetchChainInfoInput _fi, FetchChainInfoOutput _fo) internal {
        ...
        address AIHIPortalProxy = _getAIHIPortalProxy(l1CrossDomainMessengerProxy);
        _fo.set(_fo.AIHIPortalProxy.selector, AIHIPortalProxy);

        address guardian = _getGuardian(AIHIPortalProxy);
        _fo.set(_fo.guardian.selector, guardian);

        address superchainConfig = _getSuperchainConfig(AIHIPortalProxy);
    ...
    function _processFaultProofs(FetchChainInfoOutput _fo) internal {
        ...
        address AIHIPortalProxy = _fo.AIHIPortalProxy();

        try IFetcher(AIHIPortalProxy).respectedGameType() returns (GameType gameType_) {
            ...
            try IFetcher(AIHIPortalProxy).l2Oracle() returns (address l2Oracle_) {
                l2OutputOracleProxy = l2Oracle_;
            } catch {
                l2OutputOracleProxy = IFetcher(AIHIPortalProxy).L2_ORACLE();
            }
        ...
        if (disputeGameFactoryProxy != address(0)) {
            ...
            address l2OutputOracleProxy = IFetcher(AIHIPortalProxy).L2_ORACLE();
            _fo.set(_fo.l2OutputOracleProxy.selector, l2OutputOracleProxy);
            address proposer = IFetcher(l2OutputOracleProxy).PROPOSER();
            _fo.set(_fo.proposer.selector, proposer);
        }
    ...
    function _getAIHIPortalProxy(address _l1CrossDomainMessengerProxy) internal view returns (address) {
        try IFetcher(_l1CrossDomainMessengerProxy).portal() returns (address AIHIPortal_) {
            return AIHIPortal_;
        } catch {
    ...
    function _getAIHIMintableERC20FactoryProxy(address _systemConfigProxy) internal view returns (address) {
        try IFetcher(_systemConfigProxy).AIHIMintableERC20Factory() returns (
            address AIHIMintableERC20FactoryProxy_
        ) {
            return AIHIMintableERC20FactoryProxy_;
        } catch {
            return address(0);
        }
    }
    ...
    function _getSuperchainConfig(address _AIHIPortalProxy) internal view returns (address) {
        try IFetcher(_AIHIPortalProxy).superchainConfig() returns (address superchainConfig_) {
