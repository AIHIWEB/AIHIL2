...
contract L2Genesis is Deployer {
    ...
    /// @notice This predeploy is following the safety invariant #1.
    function setAIHIMintableERC20Factory() public {
        address impl = _setImplementationCode(Predeploys.AIHI_MINTABLE_ERC20_FACTORY);

        IAIHIMintableERC20Factory(impl).initialize({ _bridge: address(0) });

        IAIHIMintableERC20Factory(Predeploys.AIHI_MINTABLE_ERC20_FACTORY).initialize({
            _bridge: Predeploys.L2_STANDARD_BRIDGE
        });
    }

    /// @notice This predeploy is following the safety invariant #2,
    function setAIHIMintableERC721Factory() public {
        IAIHIMintableERC721Factory factory = IAIHIMintableERC721Factory(
            DeployUtils.create1({
                _name: "AIHIMintableERC721Factory",
                _args: DeployUtils.encodeConstructor(
                    abi.encodeCall(
                        IAIHIMintableERC721Factory.__constructor__, (Predeploys.L2_ERC721_BRIDGE, cfg.l1ChainID())
                    )
                )
            })
        );

        address impl = Predeploys.predeployToCodeNamespace(Predeploys.AIHI_MINTABLE_ERC721_FACTORY);
        console.log("Setting %s implementation at: %s", "AIHIMintableERC721Factory", impl);
    ...
    /// @notice This predeploy is following the safety invariant #1.
    ///         This contract has no initializer.
    function setAIHISuperchainERC20Factory() internal {
        _setImplementationCode(Predeploys.AIHI_SUPERCHAIN_ERC20_FACTORY);
    }

    /// @notice This predeploy is following the safety invariant #1.
    ///         This contract has no initializer.
    function setAIHISuperchainERC20Beacon() internal {
        address superchainERC20Impl = Predeploys.AIHI_SUPERCHAIN_ERC20;
        console.log("Setting %s implementation at: %s", "AIHISuperchainERC20", superchainERC20Impl);
        vm.etch(superchainERC20Impl, vm.getDeployedCode("AIHISuperchainERC20.sol:AIHISuperchainERC20"));

        _setImplementationCode(Predeploys.AIHI_SUPERCHAIN_ERC20_BEACON);
    }
