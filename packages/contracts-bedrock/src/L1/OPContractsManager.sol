...
contract OPContractsManagerUpgrader is OPContractsManagerBase {
    ...
    function upgrade(OPContractsManager.OpChainConfig[] memory _opChainConfigs) external virtual {
        ...
        for (uint256 i = 0; i < _opChainConfigs.length; i++) {
            ...
            // Grab the current respectedGameType from the AIHIPortal contract before the upgrade.
            GameType respectedGameType = IAIHIPortal(payable(opChainAddrs.AIHIPortal)).respectedGameType();

            // Grab the current SuperchainConfig from the AIHIPortal contract before the upgrade.
            ISuperchainConfig superchainConfig =
                IAIHIPortal(payable(opChainAddrs.AIHIPortal)).superchainConfig();
            ...
            // Upgrade the AIHIPortal contract implementation.
            upgradeTo(_opChainConfigs[i].proxyAdmin, opChainAddrs.AIHIPortal, impls.AIHIPortalImpl);
            ...
            // Separate context to avoid stack too deep.
            {
                ...
                IETHLockbox ethLockbox;
                {
                    ...
                    // Initialize the ETHLockbox setting the AIHIPortal as an authorized portal.
                    IAIHIPortal[] memory portals = new IAIHIPortal[](1);
                    portals[0] = IAIHIPortal(payable(opChainAddrs.AIHIPortal));
                    upgradeToAndCall(
                        _opChainConfigs[i].proxyAdmin,
                        address(ethLockbox),
                ...
                // Call `upgrade` on the AIHIPortal contract.
                IAIHIPortal(payable(opChainAddrs.AIHIPortal)).upgrade(newAnchorStateRegistryProxy, ethLockbox);
            }
...
contract OPContractsManagerDeployer is OPContractsManagerBase {
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        ...
        output.AIHIPortalProxy = IAIHIPortal(
            payable(deployProxy(_input.l2ChainId, output.opChainProxyAdmin, _input.saltMixer, "AIHIPortal"))
        );
        ...
        output.AIHIMintableERC20FactoryProxy = IAIHIMintableERC20Factory(
            deployProxy(_input.l2ChainId, output.opChainProxyAdmin, _input.saltMixer, "AIHIMintableERC20Factory")
        );
        ...
        data = encodeAIHIPortalInitializer(output, _superchainConfig);
        upgradeToAndCall(
            output.opChainProxyAdmin, address(output.AIHIPortalProxy), implementation.AIHIPortalImpl, data
        );

        // Initialize the ETHLockbox.
        IAIHIPortal[] memory portals = new IAIHIPortal[](1);
        portals[0] = output.AIHIPortalProxy;
        ...
        data = encodeAIHIMintableERC20FactoryInitializer(output);
        upgradeToAndCall(
            output.opChainProxyAdmin,
            address(output.AIHIMintableERC20FactoryProxy),
            implementation.AIHIMintableERC20FactoryImpl,
            data
        );
        ...
        returns (IResourceMetering.ResourceConfig memory resourceConfig_, ISystemConfig.Addresses memory opChainAddrs_)
    {
        ...
        opChainAddrs_ = ISystemConfig.Addresses({
            ...
            AIHIPortal: address(_output.AIHIPortalProxy),
            AIHIMintableERC20Factory: address(_output.AIHIMintableERC20FactoryProxy)
        });
        ...
        assertValidContractAddress(opChainAddrs_.AIHIPortal);
        assertValidContractAddress(opChainAddrs_.AIHIMintableERC20Factory);
    }
    ...
    /// @notice Helper method for encoding the AIHIPortal initializer data.
    function encodeAIHIPortalInitializer(
        OPContractsManager.DeployOutput memory _output,
        ISuperchainConfig _superchainConfig
    )
        internal
    ...
    /// @notice Helper method for encoding the AIHIMintableERC20Factory initializer data.
    function encodeAIHIMintableERC20FactoryInitializer(OPContractsManager.DeployOutput memory _output)
        internal
        pure
        virtual
        returns (bytes memory)
    {
        return abi.encodeCall(IAIHIMintableERC20Factory.initialize, (address(_output.l1StandardBridgeProxy)));
    }
    ...
    function encodeL1CrossDomainMessengerInitializer(
        ...
        returns (bytes memory)
    {
        return abi.encodeCall(IL1CrossDomainMessenger.initialize, (_superchainConfig, _output.AIHIPortalProxy));
    }
...
contract OPContractsManagerInteropMigrator is OPContractsManagerBase {
    ...
    function migrate(MigrateInput calldata _input) public virtual {
        ...
        IAIHIPortal[] memory portals = new IAIHIPortal[](_input.opChainConfigs.length);
        for (uint256 i = 0; i < _input.opChainConfigs.length; i++) {
            portals[i] = IAIHIPortal(payable(_input.opChainConfigs[i].systemConfigProxy.AIHIPortal()));
        }
...
contract OPContractsManager is ISemver {
    ...
    /// @notice Migrates the AIHI contracts to the latest version. This is a stub for now.
