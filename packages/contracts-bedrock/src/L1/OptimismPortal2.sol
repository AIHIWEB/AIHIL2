...
/// @custom:proxied true
/// @title AIHIPortal2
/// @notice The AIHIPortal is a low-level contract responsible for passing messages between L1
///         and L2. Messages sent directly to the AIHIPortal have no form of replayability.
///         Users are encouraged to use the L1CrossDomainMessenger for a higher-level interface.
contract AIHIPortal2 is Initializable, ResourceMetering, ReinitializableBase, ProxyAdminOwnedBase, ISemver {
    ...
    ///         withdrawal. Original AIHIPortal contract only allowed one proof to be submitted
    ...
    /// @notice Whether the AIHIPortal is using Super Roots or Output Roots.
    ...
    modifier whenNotPaused() {
        if (paused()) revert AIHIPortal_CallPaused();
        _;
    }
    ...
    /// @notice Upgrades the AIHIPortal contract to have a reference to the AnchorStateRegistry.
    ...
    /// @notice Allows the owner of the ProxyAdmin to migrate the AIHIPortal to use a new
    ///         lockbox, point at a new AnchorStateRegistry, and start to use the Super Roots proof
    ///         method. Primarily used for AIHIPortal instances to join the interop set, but
    ...
    ///         of the AIHIPortal instances connected to the new lockbox to not be able to
    ...
    function migrateToSuperRoots(IETHLockbox _newLockbox, IAnchorStateRegistry _newAnchorStateRegistry) external {
        ...
        if (msg.sender != proxyAdminOwner()) revert AIHIPortal_Unauthorized();
        ...
        if (anchorStateRegistry == _newAnchorStateRegistry) {
            revert AIHIPortal_MigratingToSameRegistry();
        }
    ...
    ///         AIHIPortal is using Super Roots (superRootsActive flag is true).
    ...
    function proveWithdrawalTransaction(
        ...
        whenNotPaused
    {
        // Make sure that the AIHIPortal is using Super Roots.
        if (!superRootsActive) {
            revert AIHIPortal_WrongProofMethod();
        }
    ...
    ///         AIHIPortal is using Output Roots (superRootsActive flag is false).
    /// @param _tx               Withdrawal transaction to finalize.
    ...
    function proveWithdrawalTransaction(
        ...
        whenNotPaused
    {
        // Make sure that the AIHIPortal is using Output Roots.
        if (superRootsActive) {
            revert AIHIPortal_WrongProofMethod();
        }
        ...
        internal
    {
        ...
        if (block.timestamp <= _disputeGameProxy.createdAt().raw()) {
            revert AIHIPortal_InvalidProofTimestamp();
        }
        ...
        if (superRootsActive) {
            ...
            if (outputRoot.chainId != systemConfig.l2ChainId()) {
                revert AIHIPortal_InvalidOutputRootChainId();
            }

            // Verify that the output root can be generated with the elements in the proof.
            if (outputRoot.root != Hashing.hashOutputRootProof(_outputRootProof)) {
                revert AIHIPortal_InvalidOutputRootProof();
            }
        } else {
            // Verify that the output root can be generated with the elements in the proof.
        ...
        if (
            ...
            revert AIHIPortal_InvalidMerkleProof();
        }
        ...
        whenNotPaused
    {
        ...
        if (l2Sender != Constants.DEFAULT_L2_SENDER) {
            revert AIHIPortal_NoReentrancy();
        }
        ...
        if (!success && tx.origin == Constants.ESTIMATION_ADDRESS) {
            revert AIHIPortal_GasEstimation();
        }
    }
    ...
    function checkWithdrawal(bytes32 _withdrawalHash, address _proofSubmitter) public view {
        ...
        if (finalizedWithdrawals[_withdrawalHash]) {
            revert AIHIPortal_AlreadyFinalized();
        }
        ...
        metered(_gasLimit)
    {
        ...
        if (_gasLimit < minimumGasLimit(uint64(_data.length))) {
            revert AIHIPortal_GasLimitTooLow();
        }
        ...
        if (_data.length > 120_000) {
            revert AIHIPortal_CalldataTooLarge();
        }
