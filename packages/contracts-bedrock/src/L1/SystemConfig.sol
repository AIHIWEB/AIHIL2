...
/// @custom:proxied true
/// @title SystemConfig
/// @notice The SystemConfig contract is used to manage configuration of an AIHI network.
...
contract SystemConfig is OwnableUpgradeable, ReinitializableBase, ISemver {
    ...
    /// @notice Storage slot that the AIHIPortal address is stored at.
    bytes32 public constant AIHI_PORTAL_SLOT = bytes32(uint256(keccak256("systemconfig.AIHIportal")) - 1);

    /// @notice Storage slot that the AIHIMintableERC20Factory address is stored at.
    bytes32 public constant AIHI_MINTABLE_ERC20_FACTORY_SLOT =
        bytes32(uint256(keccak256("systemconfig.AIHImintableerc20factory")) - 1);
    ...
    ///         Used by the AIHIPortal to meter the cost of buying L2 gas on L1.
        ...
        reinitializer(initVersion())
    {
        ...
        Storage.setAddress(AIHI_PORTAL_SLOT, _addresses.AIHIPortal);
        Storage.setAddress(AIHI_MINTABLE_ERC20_FACTORY_SLOT, _addresses.AIHIMintableERC20Factory);
    ...
    function disputeGameFactory() public view returns (address addr_) {
        IAIHIPortal2 portal = IAIHIPortal2(payable(Storage.getAddress(AIHI_PORTAL_SLOT)));
        addr_ = address(portal.disputeGameFactory());
    }

    /// @notice Getter for the AIHIPortal address.
    function AIHIPortal() public view returns (address addr_) {
        addr_ = Storage.getAddress(AIHI_PORTAL_SLOT);
    }

    /// @notice Getter for the AIHIMintableERC20Factory address.
    function AIHIMintableERC20Factory() public view returns (address addr_) {
        addr_ = Storage.getAddress(AIHI_MINTABLE_ERC20_FACTORY_SLOT);
    }
    ...
    function getAddresses() external view returns (Addresses memory) {
        return Addresses({
            ...
            AIHIMintableERC20Factory: AIHIMintableERC20Factory()
        });
    }
