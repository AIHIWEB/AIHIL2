...
/// @title AIHIMintableERC721
/// @notice This contract is the remote representation for some token that lives on another network,
///         typically an AIHI representation of an Ethereum-based token. Standard reference
///         implementation that can be extended or modified according to your needs.
contract AIHIMintableERC721 is ERC721Enumerable, ISemver {
    ...
    modifier onlyBridge() {
        require(msg.sender == BRIDGE, "AIHIMintableERC721: only bridge can call this function");
        _;
    }
    ...
    {
        require(_bridge != address(0), "AIHIMintableERC721: bridge cannot be address(0)");
        require(_remoteChainId != 0, "AIHIMintableERC721: remote chain id cannot be zero");
        require(_remoteToken != address(0), "AIHIMintableERC721: remote token cannot be address(0)");
    ...
    function supportsInterface(bytes4 _interfaceId) public view override(ERC721Enumerable) returns (bool) {
        bytes4 iface = type(IAIHIMintableERC721).interfaceId;
