...
/// @custom:proxied true
/// @title AIHISuperchainERC20
/// @notice AIHISuperchainERC20 is a standard extension of the base ERC20 token contract that unifies ERC20 token
///         bridging to make it fungible across the Superchain. This construction allows the L2StandardBridge to burn
///         and mint tokens. This makes it possible to convert a valid AIHIMintableERC20 token to a
///         AIHISuperchainERC20 token, turning it fungible and interoperable across the superchain. Likewise, it
...
contract AIHISuperchainERC20 is SuperchainERC20, Initializable {
    ...
    /// @notice Storage slot that the AIHISuperchainERC20Metadata struct is stored at.
    /// keccak256(abi.encode(uint256(keccak256("AIHISuperchainERC20.metadata")) - 1)) & ~bytes32(uint256(0xff));
    bytes32 internal constant AIHI_SUPERCHAIN_ERC20_METADATA_SLOT =
        0x07f04e84143df95a6373fcf376312ae41da81a193a3089073a54f47a74d8fb00;

    /// @notice Storage struct for the AIHISuperchainERC20 metadata.
    /// @custom:storage-location erc7201:AIHISuperchainERC20.metadata
    struct AIHISuperchainERC20Metadata {
    ...
    /// @notice Returns the storage for the AIHISuperchainERC20Metadata.
    function _getStorage() private pure returns (AIHISuperchainERC20Metadata storage storage_) {
        assembly {
            storage_.slot := AIHI_SUPERCHAIN_ERC20_METADATA_SLOT
        }
    }
    ...
    /// @notice Constructs the AIHISuperchainERC20 contract.
    constructor() {
    ...
    function initialize(
        ...
        initializer
    {
        AIHISuperchainERC20Metadata storage _storage = _getStorage();
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
        return _interfaceId == type(IAIHISuperchainERC20).interfaceId || super.supportsInterface(_interfaceId);
    }
