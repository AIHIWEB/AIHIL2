...
abstract contract StandardBridge is Initializable {
    ...
    {
        require(paused() == false, "StandardBridge: paused");
        if (_isAIHIMintableERC20(_localToken)) {
            require(
                _isCorrectTokenPair(_localToken, _remoteToken),
                "StandardBridge: wrong remote token for AIHI Mintable ERC20 local token"
            );

            IAIHIMintableERC20(_localToken).mint(_to, _amount);
        } else {
        ...
        internal
    {
        ...
        if (_isAIHIMintableERC20(_localToken)) {
            ...
            IAIHIMintableERC20(_localToken).burn(_from, _amount);
        } else {
    ...
    /// @notice Checks if a given address is an AIHIMintableERC20. Not perfect, but good enough.
    ///         Just the way we like it.
    /// @param _token Address of the token to check.
    /// @return True if the token is an AIHIMintableERC20.
    function _isAIHIMintableERC20(address _token) internal view returns (bool) {
        return ERC165Checker.supportsInterface(_token, type(ILegacyMintableERC20).interfaceId)
            || ERC165Checker.supportsInterface(_token, type(IAIHIMintableERC20).interfaceId);
    }

    /// @notice Checks if the "other token" is the correct pair token for the AIHIMintableERC20.
    ///         Calls can be saved in the future by combining this logic with
    ///         `_isAIHIMintableERC20`.
    /// @param _mintableToken AIHIMintableERC20 to check against.
    /// @param _otherToken    Pair token to check.
    /// @return True if the other token is the correct pair token for the AIHIMintableERC20.
    function _isCorrectTokenPair(address _mintableToken, address _otherToken) internal view returns (bool) {
        if (ERC165Checker.supportsInterface(_mintableToken, type(ILegacyMintableERC20).interfaceId)) {
            ...
            return _otherToken == IAIHIMintableERC20(_mintableToken).remoteToken();
        }
    }
