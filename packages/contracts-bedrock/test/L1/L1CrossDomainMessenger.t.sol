...
contract L1CrossDomainMessenger_Test is CommonTest {
    ...
    function setUp() public override {
        super.setUp();
        senderSlotIndex = ForgeArtifacts.getSlot("AIHIPortal2", "l2Sender").slot;
    ...
    function test_initialize_succeeds() external view {
        ...
        assertEq(address(l1CrossDomainMessenger.PORTAL()), address(AIHIPortal2));
        assertEq(address(l1CrossDomainMessenger.portal()), address(AIHIPortal2));
    ...
    function test_sendMessage_succeeds() external {
        // deposit transaction on the AIHI portal should be called
        vm.expectCall(
            address(AIHIPortal2),
            abi.encodeCall(
                IAIHIPortal2.depositTransaction,
                (
    ...
    function test_relayMessage_v2_reverts() external virtual {
        ...
        vm.store(address(AIHIPortal2), bytes32(senderSlotIndex), bytes32(abi.encode(sender)));
    ...
    /// @dev Tests that relayMessage reverts if caller is AIHIPortal2 and the value sent does not match the amount
    function test_relayMessage_fromOtherMessengerValueMismatch_reverts() external {
        ...
        // correctly sending as AIHIPortal but amount does not match msg.value
        vm.deal(address(AIHIPortal2), 10 ether);
        vm.prank(address(AIHIPortal2));
    ...
    /// @dev Tests that relayMessage reverts if a failed message is attempted to be replayed via the AIHIPortal2
    function test_relayMessage_fromOtherMessengerFailedMessageReplay_reverts() external {
        ...
        // cannot replay messages when AIHI portal is msg.sender
        vm.prank(address(AIHIPortal2));
        vm.expectRevert(stdError.assertionError);
        l1CrossDomainMessenger.relayMessage(
            Encoding.encodeVersionedNonce({ _nonce: 0, _version: 1 }), sender, target, 0, 0, message
    ...
    /// @dev Tests that relayMessage reverts if attempting to relay a message
    ///      with AIHIPortal as the target
    function test_relayMessage_toAIHIPortal_reverts() external {
        address sender = Predeploys.L2_CROSS_DOMAIN_MESSENGER;
        bytes memory message = hex"1111";
        ...
        vm.expectRevert("CrossDomainMessenger: cannot send message to blocked system address");
        l1CrossDomainMessenger.relayMessage(
            Encoding.encodeVersionedNonce({ _nonce: 0, _version: 1 }), sender, address(AIHIPortal2), 0, 0, message
        );
    }

    /// @dev Tests that the relayMessage function reverts if the message called by non-AIHIPortal2 but not a failed
    /// message
    ...
    function test_relayMessage_retryAfterFailure_succeeds() external {
        ...
        vm.deal(address(AIHIPortal2), value);
        vm.prank(address(AIHIPortal2));
...
contract L1CrossDomainMessenger_ReinitReentryTest is CommonTest {
    ...
    function reinitAndReenter() external payable {
        ...
        if (!attacked) {
            ...
            l1CrossDomainMessenger.initialize(ISuperchainConfig(superchainConfig), IAIHIPortal2(AIHIPortal2));
