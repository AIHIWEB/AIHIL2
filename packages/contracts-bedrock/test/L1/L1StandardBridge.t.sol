...
contract PreBridgeETH is CommonTest {
    ...
    function _preBridgeETH(bool isLegacy, uint256 value) internal {
        if (!isForkTest()) {
            assertEq(address(AIHIPortal2).balance, 0, "AIHIPortal2 balance should be 0");
        }
        uint256 nonce = l1CrossDomainMessenger.messageNonce();
        uint256 version = 0; // Internal constant in the AIHIPortal: DEPOSIT_VERSION
        ...
        // AIHIPortal emits a TransactionDeposited event on `depositTransaction` call
...
contract L1StandardBridge_DepositETH_Test is PreBridgeETH {
    ...
    ///      Calls depositTransaction on the AIHIPortal.
    ///      Only EOA can call depositETH.
    ///      ETH ends up in the AIHIPortal.
...
contract L1StandardBridge_DepositERC20_Test is CommonTest {
    ...
    // - calls AIHIPortal.depositTransaction
    ...
    function test_depositERC20_succeeds() external {
        ...
        vm.expectCall(
            address(AIHIPortal2),
            abi.encodeCall(
                IAIHIPortal2.depositTransaction, (address(l2CrossDomainMessenger), 0, baseGas, false, innerMessage)
            )
        );
...
contract L1StandardBridge_DepositERC20To_Test is CommonTest {
    ...
    function test_depositERC20To_succeeds() external {
        ...
        // The L1 XDM should call AIHIPortal.depositTransaction
        vm.expectCall(
            address(AIHIPortal2),
            abi.encodeCall(
                IAIHIPortal2.depositTransaction, (address(l2CrossDomainMessenger), 0, baseGas, false, innerMessage)
