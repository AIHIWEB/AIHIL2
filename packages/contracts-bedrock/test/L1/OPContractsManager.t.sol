...
contract OPContractsManager_Upgrade_Harness is CommonTest {
    ...
    function runUpgrade13UpgradeAndChecks(address _delegateCaller) public {
        ...
        expectEmitUpgraded(impls.AIHIPortalImpl, address(AIHIPortal2));
        expectEmitUpgraded(impls.AIHIMintableERC20FactoryImpl, address(l1AIHIMintableERC20Factory));
        ...
        assertEq(impls.AIHIPortalImpl, EIP1967Helper.getImplementation(address(AIHIPortal2)));
        assertEq(
            impls.AIHIMintableERC20FactoryImpl,
            EIP1967Helper.getImplementation(address(l1AIHIMintableERC20Factory))
        );
    ...
    function runUpgrade15UpgradeAndChecks(address _delegateCaller) public {
        ...
        // Expect the SystemConfig and AIHIPortal to be upgraded.
        expectEmitUpgraded(impls.systemConfigImpl, address(systemConfig));
        expectEmitUpgraded(impls.AIHIPortalImpl, address(AIHIPortal2));
        ...
        // Make sure that the AIHIPortal is upgraded to the right version. It must also have a
        // reference to the new AnchorStateRegistry.
        assertEq(ISemver(address(AIHIPortal2)).version(), "4.1.0");
        assertEq(impls.AIHIPortalImpl, EIP1967Helper.getImplementation(address(AIHIPortal2)));
        assertEq(address(AIHIPortal2.anchorStateRegistry()), address(newAsrProxy));
        DeployUtils.assertInitialized({
            _contractAddress: address(AIHIPortal2),
            _isProxy: true,
            _slot: 0,
            _offset: 0
        });
...
contract OPContractsManager_Upgrade_Test is OPContractsManager_Upgrade_Harness {
    ...
    function test_upgrade_duplicateL2ChainId_succeeds() public {
        ...
        Deploy deploy = Deploy(address(uint160(uint256(keccak256(abi.encode("AIHI.deploy"))))));
...
contract OPContractsManager_TestInit is Test {
    ...
    function setUp() public virtual {
        ...
        IOPContractsManager.Implementations memory impls = IOPContractsManager.Implementations({
            ...
            AIHIPortalImpl: DeployUtils.create1({
                _name: "AIHIPortal2",
                _args: DeployUtils.encodeConstructor(abi.encodeCall(IAIHIPortal2.__constructor__, (1)))
            }),
            ...
            AIHIMintableERC20FactoryImpl: DeployUtils.create1({
...
contract OPContractsManager_InteropMigrator_Test is OPContractsManager_TestInit {
    ...
    function test_migrate_withPermissionlessGame_succeeds() public {
        ...
        // Grab the two AIHIPortal addresses.
        IAIHIPortal2 AIHIPortal1 =
            IAIHIPortal2(payable(chainDeployOutput1.systemConfigProxy.AIHIPortal()));
        IAIHIPortal2 AIHIPortal2 =
            IAIHIPortal2(payable(chainDeployOutput2.systemConfigProxy.AIHIPortal()));

        // Grab the AnchorStateRegistry from the AIHIPortal for both chains, confirm same.
        assertEq(
            address(AIHIPortal1.anchorStateRegistry()),
            address(AIHIPortal2.anchorStateRegistry()),
            "AnchorStateRegistry mismatch"
        );

        // Extract the AnchorStateRegistry now that we know it's the same on both chains.
        IAnchorStateRegistry anchorStateRegistry = AIHIPortal1.anchorStateRegistry();
        ...
        // Grab the ETHLockbox from the AIHIPortal for both chains, confirm same.
        assertEq(address(AIHIPortal1.ethLockbox()), address(AIHIPortal2.ethLockbox()), "ETHLockbox mismatch");

        // Extract the ETHLockbox now that we know it's the same on both chains.
        IETHLockbox ethLockbox = AIHIPortal1.ethLockbox();

        // Check that the ETHLockbox was migrated correctly.
        assertGt(address(ethLockbox).balance, 0, "ETHLockbox balance is zero");
        assertTrue(ethLockbox.authorizedPortals(AIHIPortal1), "ETHLockbox does not have portal 1 authorized");
        assertTrue(ethLockbox.authorizedPortals(AIHIPortal2), "ETHLockbox does not have portal 2 authorized");
    ...
    function test_migrate_mismatchedSuperchainConfig_reverts() public {
        ...
        vm.mockCall(
            address(chainDeployOutput1.AIHIPortalProxy),
            abi.encodeCall(IAIHIPortal2.superchainConfig, ()),
            abi.encode(address(1234))
        );
        vm.mockCall(
            address(chainDeployOutput2.AIHIPortalProxy),
            abi.encodeCall(IAIHIPortal2.superchainConfig, ()),
            abi.encode(address(5678))
        );
