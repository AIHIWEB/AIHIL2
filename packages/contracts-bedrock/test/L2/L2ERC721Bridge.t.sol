...
contract TestMintableERC721 is AIHIMintableERC721 {
    constructor(
        address _bridge,
        address _remoteToken
    )
        AIHIMintableERC721(_bridge, 1, _remoteToken, "Test", "TST")
    { }
...
contract L2ERC721Bridge_Test is CommonTest {
    ...
    ///      with the `IAIHIMintableERC721` interface.
    function test_finalizeBridgeERC721_interfaceNotCompliant_reverts() external {
        ...
        // to be compliant with the `IAIHIMintableERC721` interface.
        vm.mockCall(
            address(l2CrossDomainMessenger),
            abi.encodeCall(l2CrossDomainMessenger.xDomainMessageSender, ()),
            abi.encode(l1ERC721Bridge)
