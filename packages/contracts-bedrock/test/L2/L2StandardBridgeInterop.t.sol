...
import { ILegacyMintableERC20 } from "src/universal/AIHIMintableERC20.sol";
...
contract L2StandardBridgeInterop_Test is CommonTest {
    ...
    function _mockDeployments(address _factory, address _token, address _deployed) internal {
        _mockAndExpect(_factory, abi.encodeCall(IAIHIERC20Factory.deployments, (_token)), abi.encode(_deployed));
    }
...
/// @notice Test suite when converting from a legacy token to a AIHISuperchainERC20 token
contract L2StandardBridgeInterop_LegacyToSuper_Test is L2StandardBridgeInterop_Test {
    /// @notice Set up the test for converting from a legacy token to a AIHISuperchainERC20 token
    ...
    function testFuzz_convert_invalidLegacyERC20Address_reverts(address _from, address _to, uint256 _amount) public {
        ...
        _mockDeployments(address(l2AIHIMintableERC20Factory), _from, address(0));
        ...
        public
    {
        ...
        _mockDeployments(address(l2AIHIMintableERC20Factory), _from, _remoteToken);

        // Mock the superchain factory to return address(0)
        _mockDeployments(address(l2AIHISuperchainERC20Factory), _to, address(0));
        ...
        public
    {
        ...
        _mockDeployments(address(l2AIHIMintableERC20Factory), _from, _fromRemoteToken);

        // Mock the superchain factory to return `_toRemoteToken`
        _mockDeployments(address(l2AIHISuperchainERC20Factory), _to, _toRemoteToken);
        ...
        public
    {
        ...
        _mockDeployments(address(l2AIHIMintableERC20Factory), _from, _remoteToken);
        _mockDeployments(address(l2AIHISuperchainERC20Factory), _to, _remoteToken);
...
/// @notice Test suite when converting from a AIHISuperchainERC20 token to a legacy token
contract L2StandardBridgeInterop_SuperToLegacy_Test is L2StandardBridgeInterop_Test {
    /// @notice Set up the test for converting from a AIHISuperchainERC20 token to a legacy token
    function _setUpSuperToLegacy(address _from, address _to) internal {
        ...
        _mockInterface(_from, type(IAIHIMintableERC20).interfaceId, false);
    }
    ...
    function testFuzz_convert_invalidLegacyERC20Address_reverts(address _from, address _to, uint256 _amount) public {
        ...
        // Mock the legacy factory to return address(0)
        _mockDeployments(address(l2AIHIMintableERC20Factory), _to, address(0));
        ...
        public
    {
        ...
        // Mock the legacy factory to return `_remoteToken`
        _mockDeployments(address(l2AIHIMintableERC20Factory), _to, _remoteToken);

        // Mock the superchain factory to return address(0)
        _mockDeployments(address(l2AIHISuperchainERC20Factory), _from, address(0));
        ...
        public
    {
        ...
        // Mock the legacy factory to return `_fromRemoteToken`
        _mockDeployments(address(l2AIHIMintableERC20Factory), _to, _fromRemoteToken);

        // Mock the superchain factory to return `_toRemoteToken`
        _mockDeployments(address(l2AIHISuperchainERC20Factory), _from, _toRemoteToken);
        ...
        public
    {
        ...
        // Mock the legacy and superchain factory to return `_remoteToken`
        _mockDeployments(address(l2AIHIMintableERC20Factory), _to, _remoteToken);
        _mockDeployments(address(l2AIHISuperchainERC20Factory), _from, _remoteToken);
