...
contract RelayActor is StdUtils {
    ...
    constructor(IAIHIPortal2 _op, IL1CrossDomainMessenger _xdm, Vm _vm, bool _doFail) {
        op = _op;
        xdm = _xdm;
        vm = _vm;
        doFail = _doFail;
    ...
    function relay(uint8 _version, uint8 _value, bytes memory _message) external {
        ...
        // Act as the AIHI portal and call `relayMessage` on the `L1CrossDomainMessenger` with
...
contract XDM_MinGasLimits is CommonTest {
    ...
    function init(bool doFail) public virtual {
        // Set up the `L1CrossDomainMessenger` and `AIHIPortal` contracts.
        super.setUp();

        // Deploy a relay actor
        actor = new RelayActor(AIHIPortal2, l1CrossDomainMessenger, vm, doFail);

        // Give the portal some ether to send to `relayMessage`
        vm.deal(address(AIHIPortal2), type(uint128).max);
...
contract XDM_MinGasLimits_Succeeds is XDM_MinGasLimits {
    ...
    ///
    ///                   - The outer min gas limit is for the call from the `AIHIPortal` to the
