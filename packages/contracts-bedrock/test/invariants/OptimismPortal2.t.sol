...
contract AIHIPortal2_Depositor is StdUtils, ResourceMetering {
    Vm internal vm;
    IAIHIPortal2 internal portal;
    bool public failedToComplete;

    constructor(Vm _vm, IAIHIPortal2 _portal) {
        vm = _vm;
        portal = _portal;
        initialize();
    }
...
contract AIHIPortal2_Invariant_Harness is CommonTest {
            ...
            payable(
                address(
                    disputeGameFactory.create(
                        AIHIPortal2.respectedGameType(), Claim.wrap(_outputRoot), abi.encode(_proposedBlockNumber)
                    )
                )
            )
        );
...
contract AIHIPortal2_Deposit_Invariant is CommonTest {
    AIHIPortal2_Depositor internal actor;

    function setUp() public override {
        super.setUp();
        // Create a deposit actor.
        actor = new AIHIPortal2_Depositor(vm, AIHIPortal2);
...
contract AIHIPortal2_CannotTimeTravel is AIHIPortal2_Invariant_Harness {
    function setUp() public override {
        super.setUp();
        ...
        targetContract(address(AIHIPortal2));
        // Exclude the proxy admin from the senders so that the proxy cannot be upgraded
        excludeSender(EIP1967Helper.getAdmin(address(AIHIPortal2)));
    }
    ...
    function invariant_cannotFinalizeBeforePeriodHasPassed() external {
        vm.expectRevert(IAIHIPortal2.AIHIPortal_ProofNotOldEnough.selector);
        AIHIPortal2.finalizeWithdrawalTransaction(_defaultTx);
    }
}

contract AIHIPortal2_CannotFinalizeTwice is AIHIPortal2_Invariant_Harness {
    function setUp() public override {
        super.setUp();
    ...
    function invariant_cannotFinalizeTwice() external {
        vm.expectRevert(IAIHIPortal2.AIHIPortal_AlreadyFinalized.selector);
        AIHIPortal2.finalizeWithdrawalTransaction(_defaultTx);
    }
}

contract AIHIPortal_CanAlwaysFinalizeAfterWindow is AIHIPortal2_Invariant_Harness {
    function setUp() public override {
        super.setUp();
