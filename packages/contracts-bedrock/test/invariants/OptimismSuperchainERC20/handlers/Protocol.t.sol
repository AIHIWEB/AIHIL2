...
import { AIHISuperchainERC20ForToBProperties } from "../helpers/AIHISuperchainERC20ForToBProperties.t.sol";
...
contract ProtocolHandler is TestBase, StdUtils, Actors {
    ...
    AIHISuperchainERC20 internal superchainERC20Impl;
    ...
    constructor() {
        ...
        superchainERC20Impl = new AIHISuperchainERC20ForToBProperties();
    ...
    function handler_mintSupertoken(uint256 index, uint96 amount) external withActor(msg.sender) {
        ...
        vm.prank(BRIDGE);
        AIHISuperchainERC20(addr).crosschainMint(currentActor(), amount);
        ...
        withActor(msg.sender)
    {
        ...
        AIHISuperchainERC20(allSuperTokens[bound(tokenIndex, 0, allSuperTokens.length)]).transfer(
    ...
    {
        vm.prank(currentActor());
        AIHISuperchainERC20(allSuperTokens[bound(tokenIndex, 0, allSuperTokens.length)]).transferFrom(
    ...
    {
        vm.prank(currentActor());
        AIHISuperchainERC20(allSuperTokens[bound(tokenIndex, 0, allSuperTokens.length)]).approve(
    ...
    function _deploySupertoken(
        ...
        internal
        returns (AIHISuperchainERC20 supertoken_)
    {
        ...
        supertoken_ = AIHISuperchainERC20(
            address(
                // TODO: Use the AIHISuperchainERC20 Beacon Proxy
                new ERC1967Proxy{ salt: hackySalt }(
                    address(superchainERC20Impl),
                    abi.encodeCall(AIHISuperchainERC20.initialize, (remoteToken, name, symbol, decimals))
                )
            )
        );
