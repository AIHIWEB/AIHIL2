...
contract ModernMockContract is BaseMockContract {
    ...
    address public AIHIMintableERC20Factory;
    ...
    function set_AIHIMintableERC20Factory(address _factory) external {
        AIHIMintableERC20Factory = _factory;
    }
...
contract FetchChainInfoTest is Test {
    ...
    function _prepareLegacyTestContext() internal returns (TestContext memory ctx_) {
        ...
        ctx_.AIHIPortal = address(new LegacyMockContract());
    ...
    function _prepareModernTestContext() internal returns (TestContext memory ctx_) {
        ...
        ctx_.AIHIPortal = address(new ModernMockContract());
    ...
    function test_legacyL2OutputOracle_succeeds() public {
        ...
        LegacyMockContract(payable(ctx.l1CrossDomainMessenger)).set_PORTAL(ctx.AIHIPortal);
        LegacyMockContract(payable(ctx.AIHIPortal)).set_L2_ORACLE(ctx.l2OutputOracle);
        LegacyMockContract(payable(ctx.AIHIPortal)).set_GUARDIAN(TEST_GUARDIAN);
        LegacyMockContract(payable(ctx.l2OutputOracle)).set_PROPOSER(TEST_PROPOSER);

        vm.mockCallRevert(
            ctx.AIHIPortal, abi.encodeCall(IRespectedGameType.respectedGameType, ()), "Function does not exist"
        );
    ...
    function test_modernPermissioned_succeeds() public {
        ...
        ModernMockContract(payable(ctx.l1CrossDomainMessenger)).set_portal(ctx.AIHIPortal);
        ModernMockContract(payable(ctx.systemConfigProxy)).set_disputeGameFactory(ctx.disputeGameFactory);
        ModernMockContract(payable(ctx.AIHIPortal)).set_superchainConfig(ctx.superchainConfig);
        ModernMockContract(payable(ctx.AIHIPortal)).set_guardian(TEST_GUARDIAN);

        ModernMockContract(payable(ctx.AIHIPortal)).set_respectedGameType(GameTypes.PERMISSIONED_CANNON);
    ...
    function test_modernPermissionless_succeeds() public {
        ...
        ModernMockContract(payable(ctx.AIHIPortal)).set_superchainConfig(ctx.superchainConfig);
        ModernMockContract(payable(ctx.AIHIPortal)).set_guardian(TEST_GUARDIAN);
        ModernMockContract(payable(ctx.AIHIPortal)).set_systemConfig(ctx.systemConfigProxy);
    ...
    function test_guardianFallback_succeeds() public {
        ...
        ctx.AIHIPortal = address(new LegacyMockContract()); // Legacy with GUARDIAN
        ...
        LegacyMockContract(payable(ctx.AIHIPortal)).set_GUARDIAN(TEST_GUARDIAN);
        LegacyMockContract(payable(ctx.AIHIPortal)).set_L2_ORACLE(address(new LegacyMockContract()));
    ...
    function test_portalFallback_succeeds() public {
        ...
        ctx.AIHIPortal = address(new LegacyMockContract());
        ...
        assertEq(ctx.output.AIHIPortalProxy(), ctx.AIHIPortal, "AIHIPortal should match PORTAL");
    }
