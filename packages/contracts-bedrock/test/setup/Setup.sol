...
import { IAIHIMintableERC721Factory } from "interfaces/L2/IAIHIMintableERC721Factory.sol";
...
import { IAIHISuperchainERC20Factory } from "interfaces/L2/IAIHISuperchainERC20Factory.sol";
...
contract Setup {
    ...
    Deploy internal constant deploy = Deploy(address(uint160(uint256(keccak256(abi.encode("AIHI.deploy"))))));

    /// @notice The address of the ForkLive contract. Set into state with `etch` to avoid
    ///         mutating any nonces. MUST not have constructor logic.
    ForkLive internal constant forkLive =
        ForkLive(address(uint160(uint256(keccak256(abi.encode("AIHI.forklive"))))));
    ...
    Artifacts public constant artifacts =
        Artifacts(address(uint160(uint256(keccak256(abi.encode("AIHI.artifacts"))))));

    L2Genesis internal constant l2Genesis =
        L2Genesis(address(uint160(uint256(keccak256(abi.encode("AIHI.l2genesis"))))));
    ...
    IAIHIMintableERC20Factory l1AIHIMintableERC20Factory;
    ...
    IAIHIMintableERC20Factory l2AIHIMintableERC20Factory =
        IAIHIMintableERC20Factory(Predeploys.AIHI_MINTABLE_ERC20_FACTORY);
    IL2ERC721Bridge l2ERC721Bridge = IL2ERC721Bridge(Predeploys.L2_ERC721_BRIDGE);
    IAIHIMintableERC721Factory l2AIHIMintableERC721Factory =
        IAIHIMintableERC721Factory(Predeploys.AIHI_MINTABLE_ERC721_FACTORY);
    ...
    IAIHISuperchainERC20Factory l2AIHISuperchainERC20Factory =
        IAIHISuperchainERC20Factory(Predeploys.AIHI_SUPERCHAIN_ERC20_FACTORY);
    ...
    function L1() public {
        ...
        AIHIPortal2 = IAIHIPortal(artifacts.mustGetAddress("AIHIPortalProxy"));
        ...
        l1AIHIMintableERC20Factory =
            IAIHIMintableERC20Factory(artifacts.mustGetAddress("AIHIMintableERC20FactoryProxy"));
    ...
    function L2() public {
        ...
        labelPredeploy(Predeploys.AIHI_MINTABLE_ERC721_FACTORY);
        ...
        labelPredeploy(Predeploys.AIHI_SUPERCHAIN_ERC20_FACTORY);
        labelPredeploy(Predeploys.AIHI_SUPERCHAIN_ERC20_BEACON);
