...
contract CrossDomainMessenger_BaseGas_Test is CommonTest {
    ...
    ///         or equal to the minimum gas limit value on the AIHIPortal.
    ///         This guarantees that the messengers will always pass sufficient
    ///         gas to the AIHIPortal.
    function testFuzz_baseGas_portalMinGasLimit_succeeds(bytes calldata _data, uint32 _minGasLimit) external view {
        ...
        uint64 minGasLimit = AIHIPortal2.minimumGasLimit(uint64(_data.length));
...
contract CrossDomainMessenger_RelayMessage_Test is CommonTest {
    ...
    function setUp() public override {
        super.setUp();
        er = new ExternalRelay(l1CrossDomainMessenger, address(AIHIPortal2));
    }

    /// @dev This test mocks an AIHIPortal call to the L1CrossDomainMessenger via
