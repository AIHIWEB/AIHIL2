...
import { AIHIMintableERC20, ILegacyMintableERC20 } from "src/universal/AIHIMintableERC20.sol";
...
contract StandardBridgeTester is StandardBridge {
    ...
    function isAIHIMintableERC20(address _token) external view returns (bool) {
        return _isAIHIMintableERC20(_token);
    }
...
/// @notice Simple implementation of the legacy AIHIMintableERC20.
contract LegacyMintable is ERC20 {
    ...
    ///         it is how the actual legacy AIHI mintable token does the
...
contract StandardBridge_Stateless_Test is CommonTest {
    ...
    AIHIMintableERC20 internal mintable;
    ...
    function setUp() public override {
        ...
        mintable = new AIHIMintableERC20({
            _bridge: address(0),
    ...
    /// @notice Test coverage for identifying AIHIMintableERC20 tokens.
    ///         This function should return true for both modern and legacy
    ///         AIHIMintableERC20 tokens and false for any accounts that
    ///         do not implement the interface.
    function test_isAIHIMintableERC20_succeeds() external view {
        // Both the modern and legacy mintable tokens should return true
        assertTrue(bridge.isAIHIMintableERC20(address(mintable)));
        assertTrue(bridge.isAIHIMintableERC20(address(legacy)));
        // A regular ERC20 should return false
        assertFalse(bridge.isAIHIMintableERC20(address(erc20)));
        // Non existent contracts should return false and not revert
        assertEq(address(0x20).code.length, 0);
        assertFalse(bridge.isAIHIMintableERC20(address(0x20)));
    }
