...
contract Initializer_Test is CommonTest {
    ...
    function setUp() public override {
        ...
        contracts.push(
            InitializeableContract({
                ...
                initCalldata: abi.encodeCall(l1CrossDomainMessenger.initialize, (superchainConfig, AIHIPortal2))
            })
        );
        ...
        contracts.push(
            InitializeableContract({
                name: "AIHIPortal2Impl",
                target: EIP1967Helper.getImplementation(address(AIHIPortal2)),
                initCalldata: abi.encodeCall(
                    AIHIPortal2.initialize, (systemConfig, superchainConfig, anchorStateRegistry, ethLockbox)
                )
            })
        );
        // AIHIPortal2Proxy
        contracts.push(
            InitializeableContract({
                name: "AIHIPortal2Proxy",
                target: address(AIHIPortal2),
                initCalldata: abi.encodeCall(
                    AIHIPortal2.initialize, (systemConfig, superchainConfig, anchorStateRegistry, ethLockbox)
                )
            })
                ...
                initCalldata: abi.encodeCall(
                    ...
                    (
                        ...
                        ISystemConfig.Addresses({
                            ...
                            AIHIPortal: address(0),
                            AIHIMintableERC20Factory: address(0)
                        }),
                        0
                    )
                )
        ...
        // AIHIMintableERC20FactoryImpl
        contracts.push(
            InitializeableContract({
                name: "AIHIMintableERC20FactoryImpl",
                target: EIP1967Helper.getImplementation(address(l1AIHIMintableERC20Factory)),
                initCalldata: abi.encodeCall(l1AIHIMintableERC20Factory.initialize, (address(l1StandardBridge)))
            })
        );
        // AIHIMintableERC20FactoryProxy
        contracts.push(
            InitializeableContract({
                name: "AIHIMintableERC20FactoryProxy",
                target: address(l1AIHIMintableERC20Factory),
                initCalldata: abi.encodeCall(l1AIHIMintableERC20Factory.initialize, (address(l1StandardBridge)))
            })
        );
        ...
        contracts.push(
            InitializeableContract({
                ...
                initCalldata: abi.encodeCall(
                    ethLockbox.initialize, (ISuperchainConfig(address(0)), new IAIHIPortal2[](0))
                )
            })
        );
    ...
    function test_cannotReinitialize_succeeds() public {
        ...
        excludes[j++] = "src/L2/AIHISuperchainERC20.sol";
